...
def main(argv):
    ...
    path = argv[0] ## 'C:\Work\HT\PRSH'
    process_id = int(argv[1]) ## 101
    process_name = argv[2] ## PRSH

    prsh_table_mapping = json.loads(config_connection.execute('''select mapping_dict from table_mapping where 
                                                            process_name='{0}' 
    ...
    for file in files:
        ...
        try:
            table_name = prsh_table_mapping[fname]
            if(fname=="PRSH_ACCT" or 
               fname == "PRSH_ACCF" or
               fname == "PRSH_GSDE" or
               fname == "PRSH_GTDE" or
               fname == "PRSH_ISCA"):
